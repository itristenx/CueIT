...
class StatusManager: ObservableObject {
    ...
    private func syncManualStatusToBackend() async {
        ...
        // Enhanced status sync with retry logic
        let kioskId = configManager.kioskId ?? "unknown"
        let maxRetries = 3
        var retryCount = 0
        
        while retryCount < maxRetries {
            let success = await APIService.shared.updateKioskStatus(
                kioskId: kioskId,
            ...
            if success {
                // Post notification for successful sync
                NotificationCenter.default.post(
                    name: .kioskStatusChanged,
        ...
        print("Failed to sync status to backend after \(maxRetries) attempts")
    }
    
    private func fetchStatusConfiguration(serverURL: String) async {
        // Enhanced status configuration fetch with error handling
        let kioskId = configManager.kioskId ?? "unknown"
        if let config = await APIService.shared.getStatusConfiguration(
            kioskId: kioskId,
            serverURL: serverURL
        ) {
            statusConfiguration = config
    ...
    private func fetchCurrentStatus(serverURL: String) async {
        // Enhanced status fetching with proper error handling
        let kioskId = configManager.kioskId ?? "unknown"
        let isActive = await APIService.shared.checkKioskActivationStatus(
            serverURL: serverURL
        )

...
struct KioskHomeView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showEmojiFeedback) {
            EmojiFeedbackView()
        }
    ...
    private var statusBar: some View {
        VStack(spacing: 20) {
            ...
            // Action Button - Changes based on kiosk mode
            actionButton
        }
    }
        ...
        Button(action: {
            ...
            .background(
                RoundedRectangle(cornerRadius: 16)
                    ...
                    .shadow(color: primaryActionColor.opacity(0.3), radius: 8, x: 0, y: 4)
            )
        }
    ...
    private var currentKioskMode: KioskMode {
        return configManager.kioskConfiguration?.mode ?? .it
    }
...
extension KioskHomeView {
    @State private var showEmojiFeedback = false
    
    var modalSheets: some View {
        Group {
            EmptyView()
                .sheet(isPresented: $showTicketForm) {
                    TicketFormView()

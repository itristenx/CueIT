/**
 * QueueIT Slack Integration Service
 * 
 * This service provides Slack bot functionality for QueueIT, allowing users to:
...
function buildModal(systems = [], urgencies = [], channel) {
  ...
  return {
    ...
    title: { type: 'plain_text', text: 'New QueueIT Ticket' },
    ...
    if (adminUrl) {
      blocks.push({
        ...
        elements: [
          {
            type: 'mrkdwn',
            text: `<${adminUrl}/tickets/${ticketId}|View Ticket> • <${adminUrl}|Open QueueIT Portal>`,
          },
        ],
      });
    }
...
app.error(async (error) => {
  console.error('QueueIT Slack App Error:', error);
});
...
(async () => {
  try {
    ...
    console.log(`✅ QueueIT Slack service running on port ${PORT}`);
    console.log(`🔗 API URL: ${process.env.API_URL}`);
    console.log(`📧 Admin URL: ${process.env.VITE_ADMIN_URL || 'Not configured'}`);
  } catch (error) {
    console.error('Failed to start QueueIT Slack app:', error);
    process.exit(1);
  }
})();

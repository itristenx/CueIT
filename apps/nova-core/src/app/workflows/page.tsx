...
interface WorkflowAction {
  id: string;
  type: 'assign' | 'notify' | 'escalate' | 'close' | 'update_status' | 'add_comment';
  label: string;
  config: {
    [key: string]: any;
  };
...
interface WorkflowTrigger {
  type: 'ticket_created' | 'ticket_updated' | 'status_changed' | 'priority_changed' | 'time_elapsed';
...
export default function WorkflowBuilderPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    department: '',
    priority: 1,
    triggerType: 'ticket_created' as WorkflowTrigger['type'],
  ...
  const handleToggleWorkflow = (workflow: WorkflowRule) => {
  ...
  const handleDeleteWorkflow = (workflow: WorkflowRule) => {
      ...
      <div className="space-y-4">
        <div>
          ...
          <Input
            ...
            placeholder="e.g., Auto-assign IT Tickets"
          />
        </div>
        ...
        <div>
          ...
          <Select value={formData.triggerType} onValueChange={(value: WorkflowTrigger['type']) => setFormData({ ...formData, triggerType: value })}>
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <div className="space-y-2">
          {formData.conditions.map((condition, index) => (
            <div key={condition.id} className="flex items-center space-x-2 p-3 border rounded">
              ...
              <Select value={condition.operator} onValueChange={(value: WorkflowCondition['operator']) => {
                const updated = [...formData.conditions];
        ...
        <div className="space-y-2">
          {formData.actions.map((action, index) => (
            <div key={action.id} className="flex items-center space-x-2 p-3 border rounded">
              <Select value={action.type} onValueChange={(value: WorkflowAction['type']) => {
                const updated = [...formData.actions];
                updated[index] = { ...action, type: value };
              ...
              <Input
                ...
                onChange={(e) => {
                  const updated = [...formData.actions];
                  updated[index] = { ...action, label: e.target.value };
                  setFormData({ ...formData, actions: updated });
                }}
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <div className="flex space-x-1">
                        ...
                        {workflow.actions.length > 3 && (
                          <span className="text-sm text-gray-500">+{workflow.actions.length - 3} more</span>
                        )}
                      </div>
                    </div>

...
export default function DepartmentsPage() {
  ...
  const handleEditDepartment = (department: Department) => {
    ...
    setFormData({
      ...
      settings: {
        ...department.settings,
        slaOverride: department.settings.slaOverride || ''
      }
    });
  };
  ...
  const handleSave = () => {
    if (isCreating) {
      const newDepartment: Department = {
        ...
        ...formData,
        managerName: formData.managerEmail ? 'New Manager' : undefined,
        userCount: 0,
        ticketCount: 0,
      ...
      const updatedDepartments = departments.map(dept =>
        dept.id === selectedDepartment.id
          ? {
              ...dept,
              ...formData,
              managerName: formData.managerEmail ? dept.managerName || 'Manager' : undefined,
              parentName: formData.parentId ? departments.find(d => d.id === formData.parentId)?.name : undefined,
              updatedAt: new Date().toISOString()
            }
          : dept
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          <div className="flex items-center space-x-2">
            <input
              type="checkbox"
              id="autoAssign"
              title="Auto-assign tickets to department members"
          ...
          <div className="flex items-center space-x-2">
            <input
              type="checkbox"
              id="escalationRules"
              title="Enable escalation rules"
              checked={formData.settings.escalationRules}
              onChange={(e) => setFormData({
                ...formData,
                settings: { ...formData.settings, escalationRules: e.target.checked }
              })}
            />
          ...
          <div className="flex items-center space-x-2">
            <input
              type="checkbox"
              id="restrictedAccess"
              title="Restricted access (HR/sensitive departments)"
        ...
        <div>
          ...
          <Input
            id="slaOverride"
            value={formData.settings.slaOverride}
            onChange={(e) => setFormData({
              ...formData,
              settings: { ...formData.settings, slaOverride: e.target.value }
            })}
            placeholder="e.g., 4h, 2h, 1h"
          />
        </div>
      </div>
    </div>
                ...
                <TableRow key={department.id}>
                  ...
                  <TableCell>
                    {department.parentName ? (
                      <span className="text-sm text-gray-600">{department.parentName}</span>
                    ) : (
                  ...
                  <TableCell>
                    <div className="flex space-x-1">
                      {department.settings.autoAssign && (
                        <Badge variant="secondary" className="text-xs">Auto</Badge>
                      )}
                      {department.settings.escalationRules && (
                        <Badge variant="secondary" className="text-xs">Escalation</Badge>
                      )}
                      {department.settings.restrictedAccess && (

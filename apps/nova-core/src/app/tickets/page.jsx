...
import { Ticket, Plus, Search, Edit, Eye, AlertTriangle, CheckCircle, Clock, XCircle, User, Calendar, Tag, MessageSquare } from 'lucide-react';
export default function TicketsPage() {
    ...
    const getPriorityBadge = (priority) => {
        ...
        return (<Badge variant={config.variant} className={config.color}>
              ...
              {filteredTickets.map((ticket) => (<TableRow key={ticket.id}>
                  <TableCell>
                    <div className="space-y-1">
                      ...
                      {ticket.tags.length > 0 && (<div className="flex flex-wrap gap-1">
                          {ticket.tags.slice(0, 3).map((tag) => (<Badge key={tag} variant="outline" className="text-xs">
                              <Tag className="h-2 w-2 mr-1"/>
                              {tag}
                            </Badge>))}
                          {ticket.tags.length > 3 && (<Badge variant="outline" className="text-xs">
                  ...
                  <TableCell>
                    {ticket.assignee ? (<div className="flex items-center space-x-2">
                        <User className="h-4 w-4 text-gray-400"/>
                        <div className="text-sm">
      ...
      <Sheet open={showViewSheet} onOpenChange={setShowViewSheet}>
        <SheetContent className="w-[600px] sm:w-[600px]">
          <SheetHeader>
            ...
            <SheetDescription>
              {selectedTicket === null || selectedTicket === void 0 ? void 0 : selectedTicket.ticketNumber} - {selectedTicket === null || selectedTicket === void 0 ? void 0 : selectedTicket.title}
            </SheetDescription>
          </SheetHeader>
...
function TicketForm({ onSubmit, onCancel }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        onSubmit(Object.assign(Object.assign({}, formData), { tags: formData.tags.split(',').map(tag => tag.trim()).filter(Boolean) }));
    };
    return (<form onSubmit={handleSubmit} className="space-y-4 mt-6">
      <div>
        <Label htmlFor="title">Title</Label>
        <Input id="title" value={formData.title} onChange={(e) => setFormData(Object.assign(Object.assign({}, formData), { title: e.target.value }))} required/>
      </div>

      <div>
        <Label htmlFor="description">Description</Label>
        <Textarea id="description" value={formData.description} onChange={(e) => setFormData(Object.assign(Object.assign({}, formData), { description: e.target.value }))} rows={4} required/>
      </div>

      <div className="grid grid-cols-2 gap-4">
        <div>
          <Label htmlFor="priority">Priority</Label>
          <Select value={formData.priority} onValueChange={(value) => setFormData(Object.assign(Object.assign({}, formData), { priority: value }))}>
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <div>
          ...
          <Input id="category" value={formData.category} onChange={(e) => setFormData(Object.assign(Object.assign({}, formData), { category: e.target.value }))}/>
        </div>
      </div>

      <div>
        <Label htmlFor="subcategory">Subcategory</Label>
        <Input id="subcategory" value={formData.subcategory} onChange={(e) => setFormData(Object.assign(Object.assign({}, formData), { subcategory: e.target.value }))}/>
      </div>

      <div>
        <Label htmlFor="tags">Tags (comma-separated)</Label>
        <Input id="tags" value={formData.tags} onChange={(e) => setFormData(Object.assign(Object.assign({}, formData), { tags: e.target.value }))} placeholder="bug, urgent, network"/>
      </div>
    ...
    return (<div className="space-y-6 mt-6">
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2 mt-1">
          {ticket.tags.map((tag) => (<Badge key={tag} variant="outline">
              {tag}
            </Badge>))}
        </div>
      </div>

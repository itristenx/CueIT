...
// Load theme configuration from theme.json (with proper error handling)
extension Theme {
    static func loadThemeConfig() -> ThemeConfig? {
        guard let themeURL = Bundle.main.url(forResource: "theme", withExtension: "json"),
              let themeData = try? Data(contentsOf: themeURL),
              let themeConfig = try? JSONDecoder().decode(ThemeConfig.self, from: themeData) else {
            return nil
        }
        return themeConfig
    }

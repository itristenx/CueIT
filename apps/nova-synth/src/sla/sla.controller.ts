...
export class SlaController {
  ...
  @ApiOperation({ summary: 'Update SLA for a ticket when priority/type changes' })
  ...
  async updateTicketSla(
    ...
    await this.slaService.updateTicketSla(ticketId, data.priority, data.type, data.category);
    return { message: 'SLA updated successfully' };
  }

  @Get('statistics')
  @ApiOperation({ summary: 'Get SLA statistics' })
  @ApiResponse({ status: 200, description: 'SLA statistics retrieved successfully' })
  ...
  async calculateSlaBreachTime(
    ...
    return {
      ...
      hoursUntilBreach: breachTime 
        ? Math.max(0, (breachTime.getTime() - createdAt.getTime()) / (1000 * 60 * 60))
        : null,
    };
  }
}

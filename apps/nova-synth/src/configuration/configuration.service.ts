...
export class ConfigurationService {
  ...
  async setConfigValue(key: string, value: any, type: string = 'string', category: string = 'general') {
  ...
  async getStatusConfig() {
    ...
    return {
      enabled: configMap.statusEnabled === '1' || configMap.statusEnabled === 'true',
      ...
      errorMessage: configMap.statusErrorMsg || 'Service temporarily unavailable',
      ...
      unavailableMessage: configMap.statusUnavailableMsg || 'Status Unavailable'
    };
  }
  ...
  async updateStatusConfig(config: any) {
    ...
    if (config.enabled !== undefined) updates['statusEnabled'] = config.enabled ? '1' : '0';
    if (config.currentStatus !== undefined) updates['currentStatus'] = config.currentStatus;
    if (config.openMessage !== undefined) updates['statusOpenMsg'] = config.openMessage;
    if (config.closedMessage !== undefined) updates['statusClosedMsg'] = config.closedMessage;
    if (config.errorMessage !== undefined) updates['statusErrorMsg'] = config.errorMessage;
    if (config.meetingMessage !== undefined) updates['statusMeetingMsg'] = config.meetingMessage;
    if (config.brbMessage !== undefined) updates['statusBrbMsg'] = config.brbMessage;
    if (config.lunchMessage !== undefined) updates['statusLunchMsg'] = config.lunchMessage;
    if (config.unavailableMessage !== undefined) updates['statusUnavailableMsg'] = config.unavailableMessage;
  ...
  async getDirectoryConfig() {
    ...
    return {
      enabled: configMap.directoryEnabled === '1' || configMap.directoryEnabled === 'true',
  ...
  async updateDirectoryConfig(config: any) {
    ...
    if (config.enabled !== undefined) updates['directoryEnabled'] = config.enabled ? '1' : '0';
    if (config.provider !== undefined) updates['directoryProvider'] = config.provider;
    ...
    if (config.bindPassword !== undefined) updates['directoryBindPassword'] = config.bindPassword;

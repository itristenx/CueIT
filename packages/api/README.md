...
An open source Express and SQLite API for help desk ticketing and configuration.
...
All API endpoints are now routed under `/api/v1`. Future changes will use `/api/v2`.

## Config Endpoints
- `GET /api/v1/config` — Get all config (except secrets)
- `PUT /api/v1/config` — Update config
- `GET /api/v1/status-config` / `PUT /api/v1/status-config` — Status messages
- `GET /api/v1/directory-config` / `PUT /api/v1/directory-config` — Directory integration
- `GET /api/v1/sso-config` / `PUT /api/v1/sso-config` — SSO config
- `GET /api/v1/scim-config` — SCIM config
- `POST /api/v1/test-smtp` — Test SMTP config
- `GET /api/v1/branding` / `PUT /api/v1/branding` — Branding config
...
## Kiosk Endpoints
- `GET /api/v1/server-info` — Kiosk setup info
- `POST /api/v1/register-kiosk` — Register kiosk
- `GET /api/v1/kiosks` — List kiosks
- `PUT /api/v1/kiosks/:id` — Update kiosk
- `POST /api/v1/kiosks/:id/refresh-config` — Refresh kiosk config
- `POST /api/v1/kiosks/:id/reset` — Reset kiosk
- `POST /api/v1/kiosks/:id/activate` — Activate kiosk
- `POST /api/v1/kiosks/:id/deactivate` — Deactivate kiosk

## Environment Variables
- Secrets (SMTP password, JWT secret, etc.) must remain in env vars.
- `VITE_API_URL` is used by frontend apps to point to the API base URL.
...
## Example Requests
See README and API docs for request/response examples.
# End of API Versioning Section

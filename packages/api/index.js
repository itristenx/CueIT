...
if (!DISABLE_AUTH) {
  app.use(
    session({
      ...
      name: 'queueit.sid' // Change default session name
    })
  );
...
// API Versioning: Setup routers for /api/v1 and /api/v2
const v1 = express.Router();
const v2 = express.Router(); // Reserved for future changes
...
// API Versioning: Mount routers under /api/v1 and /api/v2
// (v1 and v2 routers already declared above)

v1.post("/submit-ticket", ticketLimiter, validateTicketSubmission, async (req, res) => {
  ...
  const mailOptions = {
    ...
    subject: `[QueueIT Ticket] ${title} (${urgency})`,
    text: `
New IT Help Desk Request (QueueIT):
...
v1.get('/logs', ensureAuth, (req, res) => {
...
v1.delete("/logs/:id", ensureAuth, (req, res) => {
  db.deleteLog(req.params.id, (err) => {
    if (err) return res.status(500).json({ error: "DB error" });
    res.json({ message: "deleted" });
  });
});
v1.delete("/logs", ensureAuth, (req, res) => {
  db.deleteAllLogs((err) => {
    if (err) return res.status(500).json({ error: "DB error" });
    res.json({ message: "cleared" });
  });
});
v1.get("/config", ensureAuth, (req, res) => {
...
v1.put("/config", ensureAuth, (req, res) => {
...
v1.get('/status-config', ensureAuth, (req, res) => {
  db.all(
...
v1.put('/status-config', ensureAuth, (req, res) => {
...
v1.get('/directory-config', ensureAuth, (req, res) => {
  db.all(
...
v1.put('/directory-config', ensureAuth, (req, res) => {
  const updates = req.body;
  const stmt = db.prepare(
    `INSERT INTO config (key, value) VALUES (?, ?)
     ON CONFLICT(key) DO UPDATE SET value=excluded.value`
...
v1.get('/sso-config', ensureAuth, (req, res) => {
...
v1.get('/scim-config', ensureAuth, (req, res) => {
  // Check database first, fall back to environment variables
...
v1.post('/test-smtp', ensureAuth, async (req, res) => {
  ...
  try {
    const testMailOptions = {
      from: process.env.SMTP_FROM || 'noreply@queueit.local',
      to: email,
      subject: 'QueueIT SMTP Test Email',
      text: 'This is a test email from QueueIT to verify SMTP configuration is working correctly.',
      html: `
        <h2>QueueIT SMTP Test</h2>
        <p>This is a test email from QueueIT to verify SMTP configuration is working correctly.</p>
        <p>If you receive this email, your SMTP settings are configured properly.</p>
        <hr>
        <small>Sent from QueueIT Admin Panel</small>
      `
    };
...
v1.get('/feedback', ensureAuth, (req, res) => {
  db.all(`SELECT * FROM feedback ORDER BY timestamp DESC`, (err, rows) => {
    if (err) return res.status(500).json({ error: 'DB error' });
    res.json(rows);
  });
});
v1.get('/notifications', ensureAuth, (req, res) => {
  db.all(
...
v1.post('/notifications', ensureAuth, (req, res) => {
...
v1.delete('/notifications/:id', ensureAuth, (req, res) => {
...
v1.post('/verify-password', ensureAuth, (req, res) => {
...
v1.put('/admin-password', ensureAuth, (req, res) => {
  const { password } = req.body;
  if (!password) return res.status(400).json({ error: 'Missing password' });
...
v1.post('/login', apiLoginLimiter, authRateLimit, (req, res) => {
...
v1.get('/server/status', ensureAuth, (req, res) => {
  const uptime = Math.floor(process.uptime());
  const hours = Math.floor(uptime / 3600);
  const minutes = Math.floor((uptime % 3600) / 60);
  const seconds = uptime % 60;
...
v1.get('/server-info', (req, res) => {
  db.all(`SELECT key, value FROM config WHERE key IN ('organizationName', 'logoUrl', 'minPinLength', 'maxPinLength')`, (err, rows) => {
    ...
    res.json({
      ...
      serverVersion: 'QueueIT API v1'
    });
  });
});
v1.post('/register-kiosk', validateKioskRegistration, (req, res) => {
...
v1.put("/kiosks/:id", ensureAuth, (req, res) => {
  const { id } = req.params;
...
v1.get("/kiosks", ensureAuth, (req, res) => {
...
v1.delete("/kiosks/:id", ensureAuth, (req, res) => {
...
v1.delete("/kiosks", ensureAuth, (req, res) => {
...
v1.put("/kiosks/:id/active", ensureAuth, (req, res) => {
  const { id } = req.params;
...
v1.post("/kiosks/:id/refresh-config", ensureAuth, (req, res) => {
  const { id } = req.params;
...
v1.post("/kiosks/:id/reset", ensureAuth, (req, res) => {
  const { id } = req.params;
...
v1.post("/kiosks/:id/activate", ensureAuth, (req, res) => {
  const { id } = req.params;
  db.run(
...
v1.post("/kiosks/:id/deactivate", ensureAuth, (req, res) => {
  const { id } = req.params;
  db.run(
...
v1.post("/kiosks/activate", (req, res) => {
...
v1.post("/generate-activation", ensureAuth, (req, res) => {
...
// Mount v1 router for all API endpoints
app.use('/api/v1', v1);
